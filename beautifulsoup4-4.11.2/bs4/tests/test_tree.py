...
class TestFind(SoupTest):
    ...
    def test_find_everything(self):
        """Demo an optimization that finds all tags."""
        soup = self.soup("<a>foo</a><b>bar</b>")
        assert 2 == len(soup.find_all())

    def test_find_everything_with_name(self):
        """Demo an optimization that finds all tags with a given name."""
...
class TestFindAllByName(SoupTest):
    """Demo ways of finding tags by tag name."""
...
class TestParentOperations(SoupTest):
    """Demo navigation and searching through an element's parents."""
...
<script>baz</script>
 ...
 '</html>')
    ...
    def test_decompose(self):
        # Demo PageElement.decompose() and PageElement.decomposed
